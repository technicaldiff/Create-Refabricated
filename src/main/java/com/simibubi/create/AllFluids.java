...
//				.density(1400))
	...
	@Environment(EnvType.CLIENT)
	private static void makeTranslucent(RegistryEntry<? extends SimpleFlowableFluid> entry) {
		SimpleFlowableFluid fluid = entry.get();
		BlockRenderLayerMap.INSTANCE.putFluid(fluid, RenderLayer.getTranslucent());
		BlockRenderLayerMap.INSTANCE.putFluid(fluid.getStill(), RenderLayer.getTranslucent());
	}
