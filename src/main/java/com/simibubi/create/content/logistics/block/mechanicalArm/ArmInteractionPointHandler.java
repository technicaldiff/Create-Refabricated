...
public class ArmInteractionPointHandler {
	...
	public static void flushSettings(BlockPos pos) {
		...
		for (Iterator<ArmInteractionPoint> iterator = currentSelection.iterator(); iterator.hasNext();) {
			...
			if (point.pos.isWithinDistance(pos, ArmBlockEntity.getRange()))
				continue;
			iterator.remove();
			removed++;
		}
	...
	public static void tick() {
		...
		if (heldItemMainhand.isItemEqualIgnoreDamage(AllBlocks.MECHANICAL_ARM.asItem().getDefaultStack())) {
			currentItem = null;
		} else {
	...
	private static void checkForWrench(ItemStack heldItem) {
		if (!heldItem.isItemEqualIgnoreDamage(AllItems.WRENCH.getDefaultStack())) {
			return;
		}
		...
		if (!(te instanceof ArmBlockEntity)) {
			lastBlockPos = -1;
			currentSelection.clear();
			return;
		}
		...
		if (lastBlockPos == -1 || lastBlockPos != pos.asLong()) {
			currentSelection.clear();
			ArmBlockEntity arm = (ArmBlockEntity) te;
