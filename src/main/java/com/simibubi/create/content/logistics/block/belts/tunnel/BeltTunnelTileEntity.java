...
public class BeltTunnelTileEntity extends SmartBlockEntity {
	...
	//todo: LazyOptional, IItemHandler
	...
	public static BeltTunnelTileEntity create() {
		return new BeltTunnelTileEntity(AllBlockEntities.ANDESITE_TUNNEL);
	}
	...
	protected void fromTag(BlockState state, CompoundTag compound, boolean clientPacket) {
		...
		ListTag flapsNBT = compound.getList("Flaps", 3); // this is probably wrong, original is NBT.TAG_INT
		for (Tag inbt : flapsNBT)
			if (inbt instanceof IntTag)
				newFlaps.add(Direction.byId(((IntTag) inbt).getInt()));
		
		sides.clear();
		ListTag sidesNBT = compound.getList("Sides", 3); // this is probably wrong, original is NBT.TAG_INT
		...
		if (!compound.contains("TriggerFlaps"))
			return;
		flapsNBT = compound.getList("TriggerFlaps", 10); // this is probably wrong, original is NBT.TAG_COMPOUND
		for (Tag inbt : flapsNBT) {
