...
public class BeltTunnelItem extends BlockItem {
	...
	protected boolean canPlace(ItemPlacementContext ctx, BlockState state) {
		...
		return (!this.checkStatePlacement() || AllBlocks.ANDESITE_TUNNEL
	...
	protected boolean postPlacement(BlockPos pos, World world, PlayerEntity p_195943_3_, ItemStack p_195943_4_,
		...
		if (!world.isClient) {
			...
			if (belt != null) {
				...
				if (belt.casing == BeltBlockEntity.CasingType.NONE)
					belt.setCasingType(AllBlocks.ANDESITE_TUNNEL == state.getBlock() ? BeltBlockEntity.CasingType.ANDESITE : BeltBlockEntity.CasingType.BRASS);
			}
		}
		return flag;
	}
