...
public class RedstoneLinkBlock extends ProperDirectionalBlock implements IBE<RedstoneLinkBlockEntity>, BlockEntityProvider {
	...
	private void updateTransmittedSignal(BlockState state, World worldIn, BlockPos pos, Direction blockFacing) {
		...
		int transmit = power;
		withBlockEntityDo(worldIn, pos, te -> te.transmit(transmit));
	}
	...
	public int getWeakRedstonePower(BlockState state, BlockView blockAccess, BlockPos pos, Direction side) {
		if (!state.get(RECEIVER))
			return 0;
		try {
			RedstoneLinkBlockEntity tileEntity = getBlockEntity((WorldView) blockAccess, pos);
	...
	public BlockEntity createBlockEntity(BlockView world) {
		return AllBlockEntities.REDSTONE_LINK.instantiate();
	}
	...
	public ActionResult toggleMode(BlockState state, World worldIn, BlockPos pos) {
		...
		try {
			RedstoneLinkBlockEntity te = getBlockEntity(worldIn, pos);
