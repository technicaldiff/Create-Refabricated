...
public class AirFlowParticle extends AnimatedParticle {
	...
	private final AirCurrentSource source;

	protected AirFlowParticle(ClientWorld world, AirCurrentSource source, double x, double y, double z,
	...
	public static class Factory implements ParticleFactory<AirFlowParticleData> {
		...
		@Nullable
		@Override
		public Particle createParticle(AirFlowParticleData data, ClientWorld worldIn, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
			BlockEntity te = worldIn.getBlockEntity(new BlockPos(data.posX, data.posY, data.posZ));
			if (!(te instanceof AirCurrentSource))
				te = null;
			return new AirFlowParticle(worldIn, (AirCurrentSource) te, x, y, z, this.spriteSet);
		}
	}
