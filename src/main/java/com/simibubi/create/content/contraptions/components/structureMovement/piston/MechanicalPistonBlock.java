...
public class MechanicalPistonBlock extends DirectionalAxisKineticBlock implements IBE<MechanicalPistonTileEntity> {
	...
	public ActionResult onUse(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn,
		...
		if (!(player.getStackInHand(handIn)
			...
			//.isIn(Tags.Items.SLIMEBALLS)) todo: tags
			if (player.getStackInHand(handIn)
				.isEmpty()) {
				withBlockEntityDo(worldIn, pos, te -> te.assembleNextTick = true);
	...
	public BlockEntity createBlockEntity(BlockView world) {
		return AllBlockEntities.MECHANICAL_PISTON.instantiate();
	}
	...
	public static int maxAllowedPistonPoles() {
		return Create.getConfig().kinetics.maxPistonPoles;
	}
	...
	@Override
	public Class<MechanicalPistonTileEntity> getBlockEntityClass() {
		return MechanicalPistonTileEntity.class;
	}

	public static boolean isPiston(BlockState state) {
		return AllBlocks.MECHANICAL_PISTON == state.getBlock() || isStickyPiston(state);
	}

	public static boolean isStickyPiston(BlockState state) {
		return AllBlocks.STICKY_MECHANICAL_PISTON == state.getBlock();
	}

	public static boolean isExtensionPole(BlockState state) {
		return AllBlocks.PISTON_EXTENSION_POLE == state.getBlock();
	}

	public static boolean isPistonHead(BlockState state) {
		return AllBlocks.MECHANICAL_PISTON_HEAD == state.getBlock();
	}
}
