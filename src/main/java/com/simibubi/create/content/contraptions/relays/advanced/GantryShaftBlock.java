...
public class GantryShaftBlock extends DirectionalKineticBlock {
	...
	//private static final int placementHelperId = PlacementHelpers.register(new GantryPlacementHelper());
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult ray) {
		...
		GantryPlacementHelper placementHelper = (GantryPlacementHelper) PlacementHelpers.get(placementHelperId);
	...
	public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighbour, WorldAccess world,
		...
		if (direction.getAxis() != axis)
			return state;
		boolean connect = AllBlocks.GANTRY_SHAFT.getStateManager().getStates().contains(neighbour) && neighbour.get(FACING) == facing;
	...
	public BlockState getPlacementState(ItemPlacementContext context) {
		...
		BlockState clickedState =
			AllBlocks.GANTRY_SHAFT.getStateManager().getStates().contains(neighbour) ? neighbour : world.getBlockState(pos.offset(face.getOpposite()));

		if (AllBlocks.GANTRY_SHAFT.getStateManager().getStates().contains(clickedState) && clickedState.get(FACING)
	...
	public void neighborUpdate(BlockState state, World worldIn, BlockPos pos, Block p_220069_4_, BlockPos p_220069_5_,
		...
		for (BlockPos blockPos : toUpdate) {
			...
			if (te instanceof KineticBlockEntity)
				((KineticBlockEntity) te).detachKinetics();
	...
	public BlockEntity createBlockEntity(BlockView world) {
		return AllBlockEntities.GANTRY_SHAFT.instantiate();
	}
	...
	public static class GantryPlacementHelper extends PoleHelper<Direction> {

		public GantryPlacementHelper() {
			super(AllBlocks.GANTRY_SHAFT.getStateManager().getStates()::contains, s -> s.get(FACING)
				.getAxis(), FACING);
		}
