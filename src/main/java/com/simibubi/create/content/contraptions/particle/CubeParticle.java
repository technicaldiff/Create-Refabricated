...
//			RenderSystem.enableLighting();
	...
	public void tick() {
		if (this.hot && this.age > 0) {
			if (this.prevPosY == this.y) {
				billowing = true;
				/**field_21507 = false; // Prevent motion being ignored due to vertical collision TODO ?*/
	...
	public static class Factory implements ParticleFactory<CubeParticleData> {
		...
		@Nullable
		@Override
		public Particle createParticle(CubeParticleData data, ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
			CubeParticle particle = new CubeParticle(world, x, y, z, velocityX, velocityY, velocityZ);
