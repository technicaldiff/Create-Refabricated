...
public class PulleyBlockEntity extends LinearActuatorBlockEntity {
	...
	public PulleyBlockEntity() {
		super(AllBlockEntities.ROPE_PULLEY);
	}
	...
	protected void assemble() throws AssemblyException {
		...
		while (i <= maxLength) {
			...
			if (!AllBlocks.ROPE.getStateManager().getStates().contains(ropeState) && !AllBlocks.PULLEY_MAGNET.getStateManager().getStates().contains(ropeState)) {
				break;
			}
			++i;
		}
