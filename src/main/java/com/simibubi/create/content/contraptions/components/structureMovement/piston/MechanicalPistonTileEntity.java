...
public class MechanicalPistonTileEntity extends LinearActuatorBlockEntity {
	...
	@Override
	protected void toTag(CompoundTag tag, boolean clientPacket) {
		tag.putInt("ExtensionLength", extensionLength);
		super.toTag(tag, clientPacket);
	}

	public static MechanicalPistonTileEntity create() {
		return new MechanicalPistonTileEntity(AllBlockEntities.MECHANICAL_PISTON);
	}
	@Override
	...
	protected ValueBoxTransform getMovementModeSlot() {
		return new DirectionalExtenderScrollOptionSlot((state, d) -> {
			...
			Axis extensionAxis = state.get(MechanicalPistonBlock.FACING)
				.getAxis();
			Axis shaftAxis = ((Rotating) state.getBlock()).getRotationAxis(state);
