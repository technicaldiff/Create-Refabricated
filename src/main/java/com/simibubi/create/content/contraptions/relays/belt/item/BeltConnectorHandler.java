	...
	public static void tick() {
		...
		for (Hand hand : Hand.values()) {
			...
			if (!world.getBlockState(first)
				.contains(Properties.AXIS))
				continue;
			Direction.Axis axis = world.getBlockState(first)
				.get(Properties.AXIS);
			...
			Vec3d start = new Vec3d(first.getX(), first.getY(), first.getZ());
			Vec3d end = new Vec3d(selected.getX(), selected.getY(), selected.getZ());
					...
					for (int j = -1; j <= 1; j++)
						for (int k = -1; k <= 1; k++) {
							if (axis.choose(i, j, k) != 0)
								continue;
							if (axis == Direction.Axis.Y && i != 0 && k != 0)
								continue;
							if (i == 0 && j == 0 && k == 0)
								continue;
			...
			if (axis == Direction.Axis.Y && step.x != 0 && step.z != 0)
				return;
