...
public class GaugeBlock extends DirectionalAxisKineticBlock {
	...
	public BlockEntity createBlockEntity(BlockView world) {
		switch (type) {
		case SPEED:
			return AllBlockEntities.SPEEDOMETER.instantiate();
		case STRESS:
			return AllBlockEntities.STRESSOMETER.instantiate();
		default:
			return null;
		}
	}
	...
	public BlockState getPlacementState(ItemPlacementContext context) {
		...
		if (block instanceof Rotating && ((Rotating) block).hasShaftTowards(world, placedOnPos, placedOnState, face)) {
	...
	public void randomDisplayTick(BlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		...
		if (te == null || !(te instanceof GaugeBlockEntity))
			return;
		...
		for (Direction face : Iterate.directions) {
			...
			for (int i = 0; i < particleCount; i++) {
				...
				worldIn
						.addParticle(new DustParticleEffect((float) rgb.getY(), (float) rgb.getY(), (float) rgb.getZ(), 1), offset.x,
	...
	public int getComparatorOutput(BlockState blockState, World worldIn, BlockPos pos) {
		BlockEntity te = worldIn.getBlockEntity(pos);
		if (te instanceof GaugeBlockEntity) {
			GaugeBlockEntity gaugeTileEntity = (GaugeBlockEntity) te;
