...
public class ExtendoGripItemRenderer extends CustomRenderedItemModelRenderer<ExtendoGripModel> {
	...
	@Override
	protected void render(ItemStack stack, ExtendoGripModel model, PartialItemModelRenderer renderer, ModelTransformation.Mode mode, MatrixStack ms,
		VertexConsumerProvider buffer, int light, int overlay) {
		MatrixStacker stacker = MatrixStacker.of(ms);
		float animation = 0.25f;
		boolean leftHand = mode == Mode.FIRST_PERSON_LEFT_HAND;
		boolean rightHand = mode == Mode.FIRST_PERSON_RIGHT_HAND;
