...
public abstract class LaunchedItem {
	...
	public CompoundTag serializeNBT() {
		...
		c.put("Stack", stack.toTag(new CompoundTag()));
		c.put("Target", NbtHelper.fromBlockPos(target));
		return c;
	}

	public static LaunchedItem fromNBT(CompoundTag c) {
		LaunchedItem launched = c.contains("Length") ? new ForBelt()
				: c.contains("BlockState") ? new ForBlockState() : new ForEntity();
		launched.readNBT(c);
		return launched;
	}
	...
	public static class ForBlockState extends LaunchedItem {
		...
		void readNBT(CompoundTag nbt) {
			...
			if (nbt.contains("Data", NbtType.COMPOUND)) {
		...
		void place(World world) {
			...
			if (data != null) {
				...
				if (tile != null) {
					...
					if (tile instanceof KineticBlockEntity)
						((KineticBlockEntity) tile).warnOfMovement();
	...
	public static class ForEntity extends LaunchedItem {
		...
		public CompoundTag serializeNBT() {
			CompoundTag serializeNBT = super.serializeNBT();
			if (entity != null)
				serializeNBT.put("Entity", entity.toTag(new CompoundTag()));
			return serializeNBT;
		}
