...
public class GridAlignedBB {
	...
	public static GridAlignedBB fromAABB(Box aabb) {
	...
	public static GridAlignedBB fromSection(ChunkSectionPos pos) {
		return new GridAlignedBB(pos.getMinX(),
			pos.getMinY(),
			pos.getMinZ(),
			pos.getMaxX() + 1,
			pos.getMaxY() + 1,
	...
	public static Box toAABB(GridAlignedBB bb) {
	...
	public void translate(Vec3d by) {
		translate((int) by.getX(), (int) by.getY(), (int) by.getZ());
	}
	...
	public void mirrorAbout(Direction.Axis axis) {
		Vec3i axisVec = Direction.get(Direction.AxisDirection.POSITIVE, axis).getVector();
