	...
	public static Vec3d projectToPlayerView(Vec3d target, float partialTicks) {
		...
		if (mc.options.bobView) {
			...
			if (renderViewEntity instanceof PlayerEntity) {
				...
				((Vector3fAccessor) (Object) bob_translation).setY(-bob_translation.getY());  // this is weird but hey, if it works
				result3f.add(bob_translation);
			}
		}

		// ----- adjust for fov -----
		float fov = (float) ((GameRendererAccessor) mc.gameRenderer).callGetFov(ari, partialTicks, true);
