...
public interface PlacementHelper {
	 ...
	 */
	PlacementOffset getOffset(World world, BlockState state, BlockPos pos, BlockHitResult ray);
	...
	default PlacementOffset getOffset(World world, BlockState state, BlockPos pos, BlockHitResult ray, ItemStack heldItem) {
	...
	static List<Direction> orderedByDistance(BlockPos pos, Vec3d hit, Predicate<Direction> includeDirection) {
		...
		return Arrays.stream(Iterate.directions)
				...
				.map(dir -> Pair.of(dir, new Vec3d(dir.getUnitVector()).distanceTo(centerToHit)))
