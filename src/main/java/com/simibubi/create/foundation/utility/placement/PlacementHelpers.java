...
public class PlacementHelpers {
	...
	/*private static void fadedArrow(float centerX, float centerY, float r, float g, float b, float a, float length, float snappedAngle) {
		...
		bufferbuilder.vertex(0, - (10 + length), 0).color(r, g, b, a).next();

		bufferbuilder.vertex(-9, -3, 0).color(r, g, b, 0f).next();
		bufferbuilder.vertex(-6, -6, 0).color(r, g, b, 0f).next();
		bufferbuilder.vertex(-3, -8, 0).color(r, g, b, 0f).next();
		bufferbuilder.vertex(0, -8.5, 0).color(r, g, b, 0f).next();
		bufferbuilder.vertex(3, -8, 0).color(r, g, b, 0f).next();
		bufferbuilder.vertex(6, -6, 0).color(r, g, b, 0f).next();
		bufferbuilder.vertex(9, -3, 0).color(r, g, b, 0f).next();
